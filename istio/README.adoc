...
----
terrence@igloo /usr/local/istio-1.0.0
10:36:07 𝜆 kubectl -n istio-system port-forward (kubectl -n istio-system get pod -l app=grafana -o jsonpath='{.items[0].metadata.name}') 3000:3000
...
----
terrence@igloo /usr/local/istio-1.0.0
𝜆 kubectl -n istio-system port-forward (kubectl -n istio-system get pod -l app=prometheus -o jsonpath='{.items[0].metadata.name}') 9090:9090
...
----
terrence@igloo /usr/local/istio-1.0.0
𝜆 kubectl port-forward -n istio-system (kubectl get pod -n istio-system -l app=jaeger -o jsonpath='{.items[0].metadata.name}') 16686:16686
